...
{
    luaL_openlib(L, "luaaes",luaaes, 0);
    return 1;
}
...
{
    size_t pt_len=0;
    unsigned char* pt= lua_tolstring(L, 1,&pt_len);
    int ct_len=((pt_len&0xfffffffff80)+0x80);
    unsigned char* ct=(unsigned char*)malloc(ct_len);
    
    unsigned char* key= lua_tostring(L, 2);
    unsigned char* iv= lua_tostring(L, 3);
    ct_len=encrypt(pt,pt_len,key,iv,ct);
    if(ct_len>=0)
    {
        lua_pushnumber(L,0);
        lua_pushlstring(L,ct,ct_len);
        free(ct);
        return 2;
    }
    else
...
{
    size_t ct_len=0;
    unsigned char* ct= lua_tolstring(L, 1,&ct_len);
    int pt_len=ct_len;
